...
export const ReactUMG = {
    render: function(reactElement: React.ReactNode) {
        ...
        console.log("1");
        if(testWidget != undefined) {
            console.log("ok")
            let user_widget = testWidget as UE.PanelWidget;
            if(user_widget) {
                user_widget.AddChild(root.nativePtr);
                console.log("slot", user_widget.GetName());
            }
        } 
        
        return root;
    },
    init: function(inWorld: UE.World, inTestWidget: UE.Object) {
        world = inWorld;
        testWidget = inTestWidget
    }
}
